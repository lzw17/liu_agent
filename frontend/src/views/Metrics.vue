<template>
  <div class="page-wrap">
    <el-card shadow="hover" class="card">
      ...
      <el-table :data="metrics" stripe class="table" max-height="60vh">
        ...
        <el-table-column label="操作" width="120">
          <template #default="{ row }">
            <el-popconfirm title="确定删除该记录吗？" @confirm="deleteMetric(row)">
              <template #reference>
...
export default {
  ...
  methods: {
    ...
    async deleteMetric(row) {
      try {
        await axios.delete(`/api/health-assistant/metrics/${row.id}`)
